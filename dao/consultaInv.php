...
function ContadorApu() {
    ...
    $query = "(SELECT InventoryItem, StorageType, StorageBin, NumeroParte, Plant, Cantidad, StorageUnit 
    FROM ( SELECT 
    ...
    FROM Storage_Unit 
    LEFT JOIN InvSap ON Storage_Unit.Numero_Parte = InvSap.Material AND Storage_Unit.Storage_Bin = InvSap.StorageBin AND Storage_Unit.Storage_Type = InvSap.StorageType 
    WHERE InvSap.Material IS NULL 
    ORDER BY Storage_Unit.Storage_Bin) AS T 
    WHERE InventoryItem IS NOT NULL AND InventoryItem != '') 

    UNION 

    (SELECT InventoryItem, StorageType, StorageBin, NumeroParte, Plant, Cantidad, NULL AS StorageUnit 
    FROM ( SELECT 
    ...
    CASE WHEN Bitacora_Inventario.TercerConteo IS NOT NULL THEN Bitacora_Inventario.TercerConteo 
    WHEN Bitacora_Inventario.SegundoConteo IS NOT NULL THEN Bitacora_Inventario.SegundoConteo 
    ELSE Bitacora_Inventario.PrimerConteo END AS Cantidad 
    FROM Bitacora_Inventario 
    LEFT JOIN InvSap ON Bitacora_Inventario.NumeroParte = InvSap.Material AND Bitacora_Inventario.StorageBin = InvSap.StorageBin AND Bitacora_Inventario.StorageType = InvSap.StorageType 
    WHERE InvSap.Material IS NULL ) AS T 
    WHERE InventoryItem IS NOT NULL AND InventoryItem != '');";
